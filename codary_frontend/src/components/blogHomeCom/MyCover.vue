      ...
      <v-col md="4" align="start">
        <v-dialog transition="dialog-top-transition" max-width="800px" max-height="80%">
          <template v-slot:activator="{ on, attrs }">
            <v-btn class="ml-4" v-bind="attrs" v-on="on" v-show="myblogId === user.blogId">
              커버수정
            </v-btn>
          </template>
          <template v-slot:default="dialog">
            <v-card class="d-flex flex-column  pa-8">
              ...
              <v-card class="text-center mb-4">
                <v-img :src="cover" id="asd" @click="selectCover(cover)" class="selectCoverImg"></v-img>
              </v-card>
...
export default {
  ...
  methods: {
    ...
    selectCover(cover) {
      ...
      // const fianlly = document.querySelector("#asd");
      // fianlly.classList.add('consideringCover')
      // this.$refs.classList.add('consideringCover')
    },
    selectImage(img) {
      this.uploadImg = img;
      this.upload();
